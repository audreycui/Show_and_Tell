...
#TODO: should adversarial use training or inference??
class BasicS2SModel(object):
    ...
    def setup_rewards(self):
        ...
        g_loss = -tf.reduce_mean( 
            tf.reduce_sum(tf.one_hot(g_seq, self.vocab_size) * tf.log(tf.clip_by_value(g_prob, 1e-20, 1.0)), -1) * self.rewards
        )
    ...
    def get_reward(self, sess, given_tokens, conv_features, rollout_num, discriminator):
        ...
        for i in range(rollout_num):
            ...
            truth_prob = discriminator.get_truth_prob(sess, mc_sample)
