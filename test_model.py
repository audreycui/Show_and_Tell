...
class TestS2SModel(object):
    ...
    def setup_bidirection_encoder(self):
        fw_cell = single_rnn_cell(self.cell_name,self.dim_size, train_phase=self.train_phase, keep_prob=self.keep_prob_config)
        bw_cell = single_rnn_cell(self.cell_name,self.dim_size, train_phase=self.train_phase, keep_prob=self.keep_prob_config)
